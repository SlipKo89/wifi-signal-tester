...
MikroTik с модемом. Он читает качество радиоканала по SSH и никак не смешивается
с Wi-Fi-дашбордом.
...
- **Отдельная диагностика LTE**: чтение по SSH только на чтение — RSRP, RSRQ,
  SINR, RSSI/CQI при наличии, диапазон и обслуживающая сота, стабильность и
  практические советы по антенне/помехам. Подключение к Wi-Fi для этого режима
  не требуется.
